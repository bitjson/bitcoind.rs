...
pub fn connect(host: &'static str, port: u16, tx_daemon: Sender<Vec<Inventory>>) -> Socket {
    ...
    fn receiver(mut socket: Socket,
                tx: Sender<NetworkMessage>,
                tx_daemon: Sender<Vec<Inventory>>) {
        loop {
                ...
                Ok(payload) => {
                    ...
                    match message {
                        ...
                        NetworkMessage::Inv(inv) => {
                            tx_daemon.send(inv).expect("channel failure");
                        }
                        ...
                        NetworkMessage::Pong(inv) => {
                            println!("Pong {:?}", inv);
                            tx.send(message).expect("channel failure");
                        }
    ...
    fn send_version_message(mut socket: Socket,
                            version_message: VersionMessage,
                            tx_daemon: Sender<Vec<Inventory>>) {
        let network_message = NetworkMessage::Version(version_message);
        match socket.send_message(network_message) {
            Ok(()) => {
                let (tx_message_receiver, rx_message_receiver) = channel::<NetworkMessage>();
                ...
                thread::spawn(move || {
                    receiver(receiver_socket, tx_message_receiver, tx_daemon);
                });

                thread::spawn(move || {
                    pinger(sender_socket, rx_message_receiver);
                });
